...
import { cn, formatINR, formatMoney, localDateKey } from '@/lib/utils'
...
export default function LaundryBooking() {
  ...
  const visiblePrices = useMemo(() => (catalogue?.prices || []).filter((price) => {
    ...
    return garment && garment.active !== false && price.active !== false && serviceActive && !price.customer && (category === 'all' || categoryIds.has(garment.category)) && (service === 'all' || price.service === service) && `${price.garmentName} ${price.serviceName}`.toLowerCase().includes(garmentSearch.trim().toLowerCase())
  ...
  return <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <section className="h-fit rounded-[22px] border border-[#263f44]/10 bg-[#fffdf8] p-3 shadow-[0_8px_28px_rgba(37,48,43,.05)] xl:sticky xl:top-24 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto">
        ...
        <div className="mt-3 rounded-xl bg-[#123039] p-3 text-[#edf3ec]">{summaryRows({ subtotal: quoteQuery.data?.subtotal || 0, charges: quoteQuery.data?.charges || 0, discounts: quoteQuery.data?.discounts || 0, taxAmount: quoteQuery.data?.taxAmount || 0, taxRate: quoteQuery.data?.taxRate, breakdown: quoteQuery.data?.breakdown }).map((row, index) => <div key={row.key} className={cn('flex justify-between text-[11px] text-[#c4d7d0]', index > 0 && 'mt-1')}><span>{row.label}</span><span className="tabular-nums">{row.kind === 'discount' ? '−' : ''}{formatMoney(row.amount)}</span></div>)}{walletConfirmed ? <div className="mt-1 flex justify-between text-[11px] text-[#a9dcd2]"><span>LNDRY Wallet applied</span><span>−{formatMoney(walletConfirmed.amountPaise / 100)}</span></div> : null}<div className="mt-2 flex items-end justify-between border-t border-white/15 pt-2"><span className="font-serif text-base">Grand total</span><span className="font-serif text-xl tabular-nums text-[#f1ca75]">{formatMoney(quoteQuery.data?.grandTotal || 0)}</span></div>{walletConfirmed ? <div className="mt-1 flex justify-between text-xs text-[#edf3ec]"><span>Due via {paymentMode}</span><span className="font-bold tabular-nums">{formatMoney(Math.max(0, (quoteQuery.data?.grandTotal || 0) - walletConfirmed.amountPaise / 100))}</span></div> : null}</div>
